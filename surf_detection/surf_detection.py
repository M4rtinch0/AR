...
K = np.load('./Intrinsic/Intrinsic.npy')
...
def getHomography(histogram, capturePoints, canonicalPoints):    
    ...
    if histogram[bestMarker] >= 4:
        H1 = cv2.cv.CreateMat(3, 3, cv2.cv.CV_32FC1)
        mat_canonical_points1 = cv2.cv.CreateMat(len(canonicalPoints[bestMarker]), 3, cv2.cv.CV_32FC1)
        mat_capture_points1 = cv2.cv.CreateMat(len(canonicalPoints[bestMarker]), 3, cv2.cv.CV_32FC1)
        ...
        cv2.cv.FindHomography(mat_canonical_points1, mat_capture_points1, H1, cv2.cv.CV_RANSAC, 10)
        NH+=1
        
    #Get RANSCAR homography for the second best marker
    if histogram[secondBestMarker] >= 4:
        H2 = cv2.cv.CreateMat(3, 3, cv2.cv.CV_32FC1)
        mat_canonical_points2 = cv2.cv.CreateMat(len(canonicalPoints[secondBestMarker]), 3, cv2.cv.CV_32FC1)
        mat_capture_points2 = cv2.cv.CreateMat(len(canonicalPoints[secondBestMarker]), 3, cv2.cv.CV_32FC1)
        ...
        cv2.cv.FindHomography(mat_canonical_points2, mat_capture_points2, H2, cv2.cv.CV_RANSAC, 10)
        NH+=1
...
# INITIALIZE camera
camera = cv2.cv.CreateCameraCapture(0)
#cv2.cv.SetCaptureProperty(camera, cv2.cv.CV_CAP_PROP_MODE, 65)
# INITIALIZE pygame
pygame.init()
...
for i in xrange(10):
    img = cv2.cv.QueryFrame(camera)
    
image_size = cv2.cv.GetSize(img)
img_rgb = cv2.cv.CreateImage(image_size, cv2.cv.IPL_DEPTH_8U, 3)
grayscale = cv2.cv.CreateImage(image_size,cv2.cv.IPL_DEPTH_8U, 1)
...
while not mexit:
    ...
    # CAPTURE IMAGE
    img = cv2.cv.QueryFrame(camera)
    cv2.cv.CvtColor(img, img_rgb, cv2.cv.CV_BGR2RGB)
    cv2.cv.CvtColor(img_rgb, grayscale, cv2.cv.CV_BGR2GRAY)
    
    # CONVERT IMAGE to PIL
    img_pil = Image.fromstring("RGB", cv2.cv.GetSize(img_rgb),
                           img_rgb.tostring())
    ...
    #descriptors =[]
    try:
        (keypoints, descriptors) =cv2.cv.ExtractSURF(grayscale, None, cv2.cv.CreateMemStorage(),(0, 2000, 3, 2))
        # DRAW KEYPOINT

        if DRAW_DESC:
            for ((x, y), laplacian, size, mdir, hessian) in keypoints:
                radio = size*1.2/9.*2
